...
public class QuadraticProbingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        while (probe != table.length) {
            ...
            index = (initialIndex + (int) Math.pow((++probe), 2)) % table.length;
        }
        resizeBackingTable(table.length * 2 + 1);
    ...
    public V remove(K key) {
        ...
        while (probe != table.length) {
            ...
            if (!entry.isRemoved() && key.equals(entry.getKey())) {
    ...
    public void resizeBackingTable(int length) {
        if (length < size) {
            throw new IllegalArgumentException("length can't be less than the number of items in the map.");
        }
